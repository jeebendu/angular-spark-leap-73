...
import { doctorSendOtp, doctorVerifyOtp } from "@/services/authHandler";
...
export default function DoctorLogin() {
  ...
  const handleSendOtp = async () => {
    ...
    // Send OTP
    const success = await doctorSendOtp(authUser);
    if (success.status) {
      setIsOtpSent(true);
      toast({
        title: "OTP Sent",
  ...
  const handleVerifyOtp = async () => {
    ...
    const success = await doctorVerifyOtp(authUser);
    if (success.status) {
      toast({
      ...
      <div className="flex min-h-screen items-center justify-center bg-green-50">
        <div className="w-full max-w-md p-8 space-y-8 bg-white rounded-xl shadow-lg">
          ...
          <div className="text-center text-sm mt-4">
            <p>Not a doctor? <a href="/" className="text-primary font-medium">Go to patient login</a></p>
          </div>
        </div>
      </div>
    </AppLayout>
